        ...
        switch (operation) {
            ...
            case 'mine': {
                ...
                if (!Number.isInteger(limit) || limit < 1 || limit > 500)
                    return error('Usage: /memory mine [1-500]');
                const { added, scanned } = service.mine(limit);
                return success(added > 0
            ...
            case 'feedback': {
                ...
                if (tokens.length === 0) {
                    ...
                    if (!injection)
                        return success('No memory has been injected in this session yet.');
                ...
                if (/^#?\d+$/.test(first)) {
                    const injection = service.latestInjection(sessionId);
                    if (!injection)
                        return error('No memory has been injected in this session yet.');
                    const index = Number(first.replace('#', ''));
                    const claimId = injection.claimIds[index - 1];
                    if (!claimId)
                        return error(`The latest injection has ${injection.claimIds.length} claim(s); #${index} does not exist.`);
            ...
            case 'ok': {
                const sessionId = String(agent.session.id);
                const injection = service.latestInjection(sessionId);
                if (!injection || injection.claimIds.length === 0)
                    return error('No memory has been injected in this session yet.');
                const maybeIndex = tokens[0];
                const targets = maybeIndex !== undefined && /^#?\d+$/.test(maybeIndex)
                    ? [injection.claimIds[Number(maybeIndex.replace('#', '')) - 1]].filter((id) => id !== undefined)
                    : injection.claimIds;
                if (targets.length === 0)
                    return error(`The latest injection has ${injection.claimIds.length} claim(s); #${maybeIndex} does not exist.`);
                const updated = [];
                ...
                if (updated.length === 0)
                    return error('Feedback could not be recorded for any claim.');
