...
export class MemoryService {
    ...
    prepareRecall(context) {
        ...
        const closeTag = '</long_term_memory>';
        const claimIds = [];
        for (const decision of allowed) {
            ...
            if (overBudget && decision.action === 'verify')
                continue;
            // 为头部 + 前缀 + 尾标签预留空间：剩余预算才算给 content，确保第一条 use 也能注入
            // （修复：单条长 use 记忆被静默丢弃的 bug）。
            const prefix = `- [${label} #${claimIds.length + 1} ${candidate.claim.id} ${candidate.claim.kind}] `;
            const remaining = this.config.maxInjectionChars - lines.join('\n').length - prefix.length - closeTag.length - 1;
            if (remaining <= 0)
                break;
            // P2 成本分级：verify 只配短预算，use 才配剩余全宽。
            const cap = decision.action === 'verify' ? Math.min(verifyMaxChars, remaining) : remaining;
    ...
    mine(maxSessions = 50) {
        ...
        for (const claim of claims) {
            try {
                ...
                // fail-open：单条 secret-like 或非法内容跳过，不中断整批
            }
        }
     ...
     */
    mineWorkspaceOnce(sessionId, cwd) {
        if (this.minedSessions.has(sessionId))
            return { added: 0, scanned: 0 };
        this.minedSessions.add(sessionId);
        if (this.config.autoMineWorkspace === false)
            return { added: 0, scanned: 0 };
