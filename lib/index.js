...
export function apply(ctx, config) {
    ...
    try {
        ...
        ctx.logger('memory-cbdc').error('memory store unavailable; continuing without memory: %s', cause instanceof Error ? cause.message : String(cause));
    ...
    try {
        ...
        ctx.logger('memory-cbdc').warn('memory audit pruning omitted: %s', cause instanceof Error ? cause.message : String(cause));
    }
