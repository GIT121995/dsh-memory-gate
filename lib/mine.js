...
export function mineClaims(text) {
    const normalized = normalizeContent(text);
    if (normalized.length < 4 || normalized.length > 2000)
        return [];
...
export function mineSessionLog(plaintext) {
    const claims = [];
    for (const line of plaintext.split('\n')) {
        ...
        if (!text || text.includes('<long_term_memory>'))
            continue;
 ...
 */
export function mineSessions(sessionsRoot, maxSessions) {
    const claims = [];
    let scannedFiles = 0;
    try {
...
export function sessionWorkspaceKey(plaintext) {
    for (const line of plaintext.split('\n')) {
        if (!line.trim())
            continue;
        let event = null;
 ...
 */
export function mineWorkspaceSessions(sessionsRoot, workspaceKey, maxSessions) {
    const claims = [];
    let scannedFiles = 0;
    try {
        const workspaceDirs = readdirSync(sessionsRoot);
        for (const workspaceDir of workspaceDirs) {
            ...
            for (const sessionDir of sessionDirs) {
                ...
                try {
                    const plaintext = zstdDecompressSync(readFileSync(file)).toString('utf8');
                    if (sessionWorkspaceKey(plaintext) !== workspaceKey)
                        continue;
                    claims.push(...mineSessionLog(plaintext));
                }
                catch {
