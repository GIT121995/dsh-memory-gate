...
const HAN_STOP_CHARS = new Set(
  ('的了是在有我你他她它这那们个不就要也都很还更已经因为所以如果那么怎么什么为与和或而但被让把对从到等吧吗呢啊哦嗯之其及由至以于则').split(''),
)
...
for (const group of SYNONYM_GROUPS) {
  ...
  for (const member of group.members) ALIAS_TOKEN.set(member, alias)
}
 ...
 */
export function normalizeForTerms(value: string): string {
  const nfkc = value.normalize('NFKC').toLocaleLowerCase()
  let out = ''
  for (const character of nfkc) out += TRADITIONAL_TO_SIMPLIFIED[character] ?? character
  return out
}
 ...
 */
export function extractTerms(value: string, maxTerms = 120): string[] {
  ...
  const han = hanRuns.flatMap((run) => {
    ...
    if (run.length === 2) return isHanSignal(run) ? [run] : []
...
function isHanSignal(bigram: string): boolean {
...
export function mergeLearnedTerms(
  ...
  cap: number,
  baseTerms: string[] = [],
): { terms: string[]; added: string[] } {
  const blocked = new Set(baseTerms)
  const seen = new Set(existing)
  const added: string[] = []
  for (const term of incoming) {
    if (seen.has(term) || blocked.has(term)) continue
