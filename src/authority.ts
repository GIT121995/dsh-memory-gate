...
export function decideAuthority(
  ...
  if (candidate.recallChannel === 'capsule') reasonCodes.push('trusted_global_capsule')
  ...
  if (beliefScore < policy.minUseBelief) reasonCodes.push('belief_below_use_threshold')
  if (riskScore > policy.maxUseRisk) reasonCodes.push('risk_above_use_threshold')
  if (claim.origin === 'heuristic') reasonCodes.push('heuristic_origin')
  if (
    reasonCodes.includes('belief_below_use_threshold') ||
  ...
  function make(action: AuthorityDecision['action']): AuthorityDecision {
    return {
      claimId: claim.id,
      action,
      reasonCodes,
...
function baseRisk(kind: SearchCandidate['claim']['kind']): number {
  switch (kind) {
    case 'preference':
      return 0.12
    case 'fact':
